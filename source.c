...
void display(int numbers[], int size){                    //Print all elements of the array in one line
    int c=0;
    while(size!=c){                                      //Loop based on size
        printf(" %d",numbers[c]);
        c++;
    }
}
...
void compare_sets(int set1[], int set2[], int size){      //Compare two arrays
    ...
    if (mismatch_count>=1){                                       //If mismatches found
        printf("\n\nTwo integer sets are not identical\n");
        printf("Total mismatches found: \n%d",mismatch_count);
    }else{                                                        //If no mismatches found
...
void display_pipes(int pnumbers[], int psize){                    //Display numbers with pipes

    printf("\n\nLet's print numbers with pipes\n");
    get_input(pnumbers, psize);                                   //Getting numbers from user
    display(pnumbers, psize);                                     //Displaying numbers
    char set[]="||||| ";                                          //String for 5-pipe block

    for (int i=0; i<psize; i++){
        int n=pnumbers[i];                                          //Taking the value of element
        printf("\n%2d    ",n);                                      //%2d = print number in 2-width field.

                int f=n/5;                                          //Number of 5-pipe block
                int r=n%5;                                          //Remaining pipes

                while(f>0){                                         //Loop to print 5-pipe blocks
                    printf("%s",set);
                    f--;
                }
                while(r>0){                                         //Loop to print remaining pipes
                    printf("|");
                    r--;
                }
